...
/// Nash
/// </summary>
public class MotionMatchingBakedEditor : EditorWindow
{
    [MenuItem("Tools/BakeMotion")]
    static void MakeWindow()
    {
        window = GetWindow(typeof(MotionMatchingBakedEditor)) as MotionMatchingBakedEditor;
        window.oColor = GUI.contentColor;
    }

    private static MotionMatchingBakedEditor window;
    ...
    {
        MoMaSetting motionMatcherSettings = AssetDatabase.LoadAssetAtPath(MotionMatcherSettingsPath, typeof(MoMaSetting)) as MoMaSetting;
        if (motionMatcherSettings)
        {
            predictionTrajectoryTimeList = predictionTimeList(motionMatcherSettings.Second, motionMatcherSettings.SaveInSecond);
            captureBoneList = motionMatcherSettings.KeyJoints;
    ...
    private float[] predictionTimeList(float second, float saveinsecond)
    {
        var timeList = new List<float>();
        for(int i = 0; i < saveinsecond; i++)
        {
            timeList.Add(second/saveinsecond * i);
        }
    ...
    private void CreateSnapshots()
    {
        ...
        AnimationClips meshAnimationList = ScriptableObject.CreateInstance<AnimationClips>();
        meshAnimationList.AnimClips = new AnimClip[clips.Count*2].ToList();
        ...
        {
            AnimationClip animClip = clips[(int)(x*1f/2)];
            ...
            meshAnimationList.AnimClips[x] = new AnimClip();
            AnimClip motionData = meshAnimationList.AnimClips[x];
            motionData.Name = animClip.name;
            ...
            motionData.Frames = new AnimationFrame[totalFrame].ToList();
        ...
        for (int j = 0; j < clips.Count*2; j++)
        {
            ...
            AnimationClip animClip = clips[(int)(j * 1f / 2)];
            ...
            sampleGO.transform.position = Vector3.zero;
            sampleGO.transform.rotation = Quaternion.identity;
            animClip = clips[(int)(j * 1f / 2)];
            motionData = meshAnimationList.AnimClips[j];
            bakeFrames = Mathf.CeilToInt(animClip.length * fps);
            frame = 0;
            for (int i = 0; i < bakeFrames; i += frameSkips[animClip.name])
            {
                float bakeDelta = Mathf.Clamp01(((float)i / bakeFrames));
    ...
    private void CaptureBoneSnapShot(AnimationClip animClip, AnimationFrame motionFrameData, AnimationFrame lastMotionFrameData, GameObject sampleGO, float bakeFrames, float CurrentFrame)
    {
    ...
    private void CaptureBoneSnapShotInPlace(AnimationClip animClip, AnimationFrame motionFrameData, AnimationFrame lastMotionFrameData, GameObject sampleGO, float bakeFrames, float CurrentFrame)
    {
        float frameSkipsTimeStep = frameSkips[animClip.name] / (float)fps;
        float bakeDelta = CurrentFrame / bakeFrames;
        float animationTime = bakeDelta * animClip.length;
    ...
    private void CaptureTrajectorySnapShot(AnimationClip animClip, AnimationFrame motionFrameData, AnimationFrame lastMotionFrameData, GameObject sampleGO, float bakeFrames, float currentFrame)
    {
        ...
        for (int i = 0; i < predictionTrajectoryTimeList.Length; i++)
        {
            ...
            TempMotionTrajectory motionTrajectoryData = motionFrameData.tempMotionTrajectory[i];
        ...
        TempMotionTrajectory firstMotionTrajectoryData = motionFrameData.tempMotionTrajectory[0];
        TempMotionTrajectory lastMotionTrajectoryData = motionFrameData.tempMotionTrajectory[motionFrameData.tempMotionTrajectory.Length - 1];
